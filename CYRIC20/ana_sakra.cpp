...
int main(int argc, char *argv[]){
  ...
  while(!fin.eof()){
    ...
    case RIDF_EVENT_TS:
      if(evt_stored){
	if(tmp_flag==1 && tmp_flag2==1){
	  tree->Fill();
	}
	evt_stored=false;
	//	cout << tmp_flag << tmp_flag2 << endl; 
	...
	for(int i=0;i<RECORD_LENGTH;i++){
	  ...
	  if(evtn%100==0) waveform[board][ch]->Fill(i,data[board][ch][i],1.);
      ...
      if(board==1){
	for(int j=0;j<16;j++){
	  if(Energy[board][j]<1.) continue;
	  tmp_flag=1;
	}
      }
      if(board==0){
	for(int j=0;j<16;j++){
	  if(Energy[board][j]<1. || Energy[board][j]>15.) continue;
	  tmp_flag2=1;
	}
      }
