...
//*** for Analyzer time measurement ***
#include <time.h>
#include <sys/time.h>
struct timeval s_time_ana, e_time_ana, diff_time_ana;
struct timeval s_time_fill, e_time_fill, diff_time_fill;
struct timeval s_time_loop, e_time_loop, diff_time_loop;
struct timeval s_time_root, e_time_root, diff_time_root;
...
//vector<uint16_t> temp_data[N_FADC_BOARD][N_CH];
//array<array<vector<uint16_t>,N_CH>,N_FADC_BOARD> temp_data;
vector<vector<uint16_t>> temp_data[N_FADC_BOARD];
...
int main(int argc, char *argv[]){
  diff_time_loop.tv_sec=0;
  diff_time_loop.tv_usec=0;
  diff_time_fill.tv_sec=0;
  diff_time_fill.tv_usec=0;
  diff_time_root.tv_sec=0;
  diff_time_root.tv_usec=0;
  gettimeofday(&s_time_ana,NULL);
  ...
  for(int i=0;i<N_FADC_BOARD;++i){
    temp_data[i].resize(N_CH);
    for(int j=0;j<N_CH;++j) temp_data[i][j].reserve(RECORD_LENGTH);
  }
  
  gettimeofday(&s_time_root,NULL);
  ...
  //tree->Branch("data","std::array<std::arraystd::vector<int>",&temp_data);
  //tree->Branch("data","std::array<std::array<std::vector<uint16_t>,16>,3>",&temp_data);
  //auto branch=tree->Branch("data",temp_data);

  for(int i=0;i<N_FADC_BOARD;++i) tree->Branch(Form("data_%d",i),"std::vector<std::vector<uint16_t>>",&temp_data[i]);
  ...
  while(!fin.eof()){
    ...
    case RIDF_EVENT_TS:
      if(evt_stored){
	gettimeofday(&s_time_fill,NULL);
	//branch->SetAddress(temp_data);
	tree->Fill();
	gettimeofday(&e_time_fill,NULL);
	timersub(&e_time_fill,&s_time_fill,&temp_time);
	timeradd(&diff_time_fill,&temp_time,&diff_time_fill);
	evt_stored=false;
      }
	...
	temp_data[board][ch].clear();
	//temp_data[board].clear();
	...
	for(int i=0;i<(ch_size-1);++i){
	  if((buff[bp]>>31)  & 0x1){
	    ...
	    //for(uint j=0; j<skipped_samples; j++,data_index++) temp_data[board][ch].push_back(0);
	    for(uint j=0; j<skipped_samples; j++,data_index++) temp_data[board][ch].emplace_back(0);
	    //for(uint j=0; j<skipped_samples; j++,data_index++) temp_data[board].push_back(0);
	    continue;
	  }
	  /*
	  ...
	  */
	  /*
	  temp_data[board][ch].push_back(buff[bp] & 0x00003FFF);
	  temp_data[board][ch].push_back((buff[bp++] & 0x3FFF0000)>>16);
	  */
	  temp_data[board][ch].emplace_back(buff[bp] & 0x00003FFF);
	  temp_data[board][ch].emplace_back((buff[bp++] & 0x3FFF0000)>>16);
	  if(400<i&&i<500) baseline_ave[board][ch]+=temp_data[board][ch][i]/100; //	  

	  /*
	  temp_data[board].push_back(buff[bp] & 0x00003FFF);
	  temp_data[board].push_back((buff[bp++] & 0x3FFF0000)>>16);
	  if(400<i&&i<500) baseline_ave[board][ch]+=temp_data[board][i]/100; //	  
	  */
	}
	
	if(temp_data[board][ch][0]>baseline[board][ch]) continue;
	//if(temp_data[board][0]>baseline[board][ch]) continue;
	//for(int i=0;i<RECORD_LENGTH;i++){
	for(int i=0;i<(int)temp_data[board][ch].size();i++){
	  //for(int i=0;i<(int)temp_data[board].size();i++){
	  if(temp_data[board][ch][i]==0) continue;
	  //if(temp_data[board][i]==0) continue;
	  //if(evtn%50==0) waveform[board][ch]->Fill(i,temp_data[board][ch][i],1.);
	  waveform[board][ch]->Fill(i,temp_data[board][ch][i],1.);
	  //waveform[board][ch]->Fill(i,temp_data[board][i],1.);
	  if(ADC[board][ch]<abs(temp_data[board][ch][i]-baseline[board][ch])){
	    //if(ADC[board][ch]<abs(temp_data[board][i]-baseline[board][ch])){
	    ADC[board][ch]=abs(temp_data[board][ch][i]-baseline[board][ch]);
	    //ADC[board][ch]=abs(temp_data[board][i]-baseline[board][ch]);
	...
	*/
	...
	//	}
      ...
      gettimeofday(&e_time_loop,NULL);
      timersub(&e_time_loop,&s_time_loop,&temp_time);
      timeradd(&diff_time_loop,&temp_time,&diff_time_loop);
  ...
  cout << endl;
  cout << "Analyzed event    : " << evtn << endl;
  gettimeofday(&e_time_ana,NULL);
  timersub(&e_time_ana,&s_time_ana,&diff_time_ana);
  cout << "Analyzer time     : " << (double)diff_time_ana.tv_sec+(double)diff_time_ana.tv_usec/1000000 << "[s]" << endl;
  timersub(&e_time_root,&s_time_root,&diff_time_root);
  cout << "Root setting time : " << (double)diff_time_root.tv_sec+(double)diff_time_root.tv_usec/1000000 << "[s]" << endl;
  cout << "Tree Fill time    : " << (double)diff_time_fill.tv_sec+(double)diff_time_fill.tv_usec/1000000 << "[s]" << endl;
  cout << "Segment loop time : " << (double)diff_time_loop.tv_sec+(double)diff_time_loop.tv_usec/1000000 << "[s]" << endl;
