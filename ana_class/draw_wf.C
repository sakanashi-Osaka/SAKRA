...
int draw_wf(int board, int ich){
  TFile *f=new TFile("test.root");
  TTree *t=(TTree*)f->Get("tree");
  vector<TV1730ZLE> *event=new vector<TV1730ZLE>(2);
  ...
  h=new TH2D("h","h",4096,0,4096,4000,0,16000);
  for(int  i=0;i<n;i++){
    t->GetEntry(i);
    for(int j=0;j<event->at(board).ch[ich].data.size();++j){
      h->Fill(j,event->at(board).ch[ich].data[j]);
    }
  }
